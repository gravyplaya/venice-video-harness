...
export interface CharacterLock {
  ...
  /**
   * Model that generated each reference angle. Used by the Seedance 2.0
   * pre-flight gate to confirm refs were produced by a compatible family
   * (e.g. `seedream-v5-lite`). Missing entries are treated as "unknown".
   */
...
function serializeLock(lock: CharacterLock): SerializedCharacterLock {
  ...
  return {
    ...
    locked: lock.locked,
    seed: lock.seed,
    referenceImageModels: lock.referenceImageModels,
  };
}
...
function deserializeLock(
  ...
  return {
    ...
    locked: data.locked,
    seed: data.seed,
    referenceImageModels: data.referenceImageModels,
  };
}
...
export class ReferenceManager {
  ...
  async generateReferences(
    ...
    const lock: CharacterLock = {
      ...
      referenceImageModels: {
        front: DEFAULT_MODEL,
        threeQuarter: DEFAULT_MODEL,
        profile: DEFAULT_MODEL,
        fullBody: DEFAULT_MODEL,
      },
    };
  ...
  async saveReferences(lock: CharacterLock): Promise<void> {
    ...
    // Write reference images as PNG files and emit a provenance sidecar
    // for each so the Seedance pre-flight gate can validate later.
    for (const angle of REFERENCE_ANGLES) {
      const imagePath = join(dir, angle.filename);
      ...
      const modelUsed = lock.referenceImageModels?.[angle.key];
      if (modelUsed) {
        // Character reference sheets always depict a human face, so Seedance
        // cares about their provenance.
        await writeImageProvenance(imagePath, modelUsed, [], { hasFace: true });
      }
    }
